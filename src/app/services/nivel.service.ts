/**
 * @file nivel.service.ts
 * @description Gestiona el contenido de los 4 niveles académicos.
 * Cada nivel se persiste de forma independiente en localStorage.
 */
import { Injectable } from '@angular/core';
import { NivelConfig } from '../models/nivel.model';

const KEY = (id: string) => `edu_nivel_${id}`;
...
export class NivelService {
  ...
  get(nivelId: string): NivelConfig {
    const raw = localStorage.getItem(KEY(nivelId));
    const def = DEFAULTS[nivelId] ?? DEFAULTS['preparatoria'];
    return raw ? { ...def, ...JSON.parse(raw) } : { ...def };
  }

  getCopia(nivelId: string): NivelConfig {
    return JSON.parse(JSON.stringify(this.get(nivelId)));
  }

  guardar(config: NivelConfig): void {
    localStorage.setItem(KEY(config.nivelId), JSON.stringify(config));
  }
