...
export interface NosotrosConfig {
  ...
  // Autoridades (solo textos de sección)
...
const DEFAULT: NosotrosConfig = {
  ...
  misionVision: [
    {
      ...
      icon: 'target',
      title: 'Misión',
      description: 'Formar personas íntegras con valores salesianos, competencias técnicas y científicas que contribuyan al desarrollo sostenible de la sociedad.',
    },
    {
      id: 2,
      icon: 'eye',
      title: 'Visión',
      description: 'Ser una institución educativa de referencia nacional, reconocida por la excelencia académica, la innovación pedagógica y el compromiso con la comunidad.',
    },
  ],
  ...
  valores: [
    { id: 1, icon: 'heart',    title: 'Amor',        description: 'Actuamos con amor salesiano en cada interacción con nuestra comunidad educativa.' },
    ...
    { id: 3, icon: 'users',    title: 'Comunidad',   description: 'Fomentamos el sentido de pertenencia y la colaboración entre todos los miembros.' },
    { id: 4, icon: 'book',     title: 'Sabiduría',   description: 'Promovemos el conocimiento crítico y la formación integral del ser humano.' },
  ],
...
export class NosotrosService {
  ...
  getCopia(): NosotrosConfig { return JSON.parse(JSON.stringify(this.get())); }
  guardar(c: NosotrosConfig): void { localStorage.setItem(KEY, JSON.stringify(c)); }
