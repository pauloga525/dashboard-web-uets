...
const DEFAULT: AdmisionesConfig = {
  ...
  heroDescripcion: 'Tu futuro comienza aquí. Únete a una comunidad de innovadores, líderes y pensadores. Las solicitudes están abiertas para todos los programas de bachillerato.',
  heroImagenFondo: '',
  ...
  ctaDescripcion: 'No pierdas la oportunidad de unirte a nuestra comunidad. La solicitud toma menos de 30 minutos para completar.',
...
export class AdmisionesService {
  get(): AdmisionesConfig {
    const raw = localStorage.getItem(KEY);
    return raw ? { ...DEFAULT, ...JSON.parse(raw) } : { ...DEFAULT };
  }
  getCopia(): AdmisionesConfig { return JSON.parse(JSON.stringify(this.get())); }
  guardar(c: AdmisionesConfig): void { localStorage.setItem(KEY, JSON.stringify(c)); }
